...
public class ConsumeFluid extends Consume{
    ...
    public void display(Stats stats){
        if(amount != 0){
            if(continuous){
                stats.add(amount > 0 ? Stat.input : Stat.output, OlStatValues.fluid(fluid, Math.abs(amount), 1f, true));
            }else{
                stats.add(amount > 0 ? Stat.input : Stat.output, OlStatValues.fluid(fluid, Math.abs(amount), 60f, false));
            }
        }
...
//        return startRange <= build.getPressure(fluid) && build.getPressure(fluid) <= endRange && (fluid == null || build.getFluid(fluid) > amount);
        return startRange <= build.getPressure(fluid) && build.getPressure(fluid) <= endRange;
    }
